...
parser = argparse.ArgumentParser(description="Text Classification with N-gram TF-IDF Vectorization using Naive Bayes")
...
# Extract feature log probabilities (indicative of importance in each class)
feature_log_prob = clf.feature_log_prob_

# Calculate the difference between the two classes' log probabilities for each feature
feature_diff = feature_log_prob[1] - feature_log_prob[0]

# Simulate a coefficient-like structure using log probabilities difference
feature_coeff = {feature_names[i]: feature_diff[i] for i in range(len(feature_names))}
...
print(tabulate(top_positive_features, headers=['Feature', 'Log Probability Difference'], tablefmt='grid'))

print("\nTop 10 Negative Features:")
print(tabulate(top_negative_features, headers=['Feature', 'Log Probability Difference'], tablefmt='grid'))

# Save feature importance
with open(f"{file_prefix}_feature_importances.txt", 'w') as f:
    f.write("Top 10 Positive Features:\n")
    f.write(tabulate(top_positive_features, headers=['Feature', 'Log Probability Difference'], tablefmt='grid'))
    f.write("\n\nTop 10 Negative Features:\n")
    f.write(tabulate(top_negative_features, headers=['Feature', 'Log Probability Difference'], tablefmt='grid'))
