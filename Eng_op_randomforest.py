...
parser = argparse.ArgumentParser(description="Text Classification with N-gram TF-IDF Vectorization using Random Forest")
...
# Random Forest Model
clf = RandomForestClassifier(n_estimators=100, random_state=0)  # Using 100 trees
...
# Simulate a coefficient-like structure using feature importances
coefficients = importances.tolist()
...
plot_feature_importance(top_positive_features, 'Top 10 Important Features', f"{file_prefix}_top_positive_features.png")
plot_feature_importance(top_negative_features, 'Top 10 Least Important Features', f"{file_prefix}_top_negative_features.png")
...
print(tabulate(top_negative_features, headers=['Feature', 'Importance'], tablefmt='grid'))
...
with open(f"{file_prefix}_feature_importances.txt", 'w') as f:
    ...
    f.write("\n\nTop 10 Least Important Features:\n")
    f.write(tabulate(top_negative_features, headers=['Feature', 'Importance'], tablefmt='grid'))
