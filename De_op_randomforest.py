...
neutral_sentences = []
for paragraph in neutral_data['text_de']:
...
neutral_df = pd.DataFrame(neutral_sentences, columns=['text_de'])
...
combined_data['token_count'] = combined_data['text_de'].apply(count_tokens)

# Remove stopwords
stop_words = stopwords.words('german')
combined_data['text_de'] = combined_data['text_de'].apply(lambda x: ' '.join([word for word in x.split() if word.lower() not in stop_words]))
...
X = tfidf.fit_transform(filtered_data['text_de'])
...
# Random Forest Model
clf = RandomForestClassifier(random_state=0, n_estimators=100, max_depth=10).fit(X_train, y_train)
...
feature_sorted = sorted(feature_importance.items(), key=itemgetter(1), reverse=True)

# Top 10 Positive and Negative Features
top_positive_features = feature_sorted[:11]
...
def plot_feature_importance(features, title, filename):
    ...
    plt.barh([feature for feature, _ in features], [importance for _, importance in features], color='skyblue')
    plt.title(title)
...
plot_feature_importance(top_positive_features, 'Top 10 Important Features', f"{file_prefix}_top_important_features.png")

# Display results in tabular form using tabulate
print("\nTop 10 Important Features:")
print(tabulate(top_positive_features, headers=['Feature', 'Importance'], tablefmt='grid'))

# Save feature importances
with open(f"{file_prefix}_feature_importances.txt", 'w') as f:
    f.write("Top 10 Important Features:\n")
    f.write(tabulate(top_positive_features, headers=['Feature', 'Importance'], tablefmt='grid'))
